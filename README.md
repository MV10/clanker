...
Automated messages are always ignored. These are what phone carriers call A2P 10DLC (Automation-to-Person), which are messages sent from an all-numeric "name" up to 10 digits (for example, Amazon deliveries, bank withdrawals, 2-factor auth codes, flavor-of-the-week political campaigns, etc). If _any_ participant name is all-numeric (up to 10 digits), the conversation is ignored. Message content will not be read, and you can't activate the AI in those conversations. Only the Settings context-menu item is available when this type of conversation is active.
