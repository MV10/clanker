...
* Add context menu item "Show Conversation State (Sanitized)" as the second menu item under Diagnostics. This works like the existing Show Conversation State option, but with sensitive data redacted for public sharing for support purposes. Change participant names to User001 amd User002 etc, and change phone number participant names to (001) XXX-XXX and (002) XXX-XXXX etc, also redact image blob URI GUIDs with Xs, and change URL domains to "redacted.com". Also redact data in message content and the LLM's summary and customization fields.
